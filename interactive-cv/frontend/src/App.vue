  ...
  <div class="relative font-sans dark:text-dark-text text-gray-800">
    ...
    <div class="fixed inset-0 -z-20 bg-gray-100 dark:bg-dark-background overflow-hidden">
      <div class="hidden dark:block">
        <!-- Galaxy background -->
        <div class="absolute inset-0 opacity-20 animate-galaxy-shift bg-[radial-gradient(circle,_#0a0f21,_#0d1117,rgba(13,17,23,0))]" />

        <!-- Stars -->
        <template v-for="(star, i) in stars" :key="'star-' + i">
          <div
            class="absolute rounded-full bg-dark-primary animate-star-twinkle"
        ...
        <template v-for="m in clickMeteors" :key="m.id">
          <span
            class="absolute bg-white w-[2px] h-[2px] rounded-full animate-meteor"
            :style="{
              top: m.y + 'px',
              left: m.x + 'px',
        ...
        <template v-for="p in particles" :key="p.id">
          <div
            class="absolute rounded-full pointer-events-none transition-all duration-500 ease-out"
            :style="{
              width: p.size,
              height: p.size,
              background: p.color,
